...
function dataUpdate() {
  let newData = [];
  newData.push(
    dashboardStore.jobResultAnalysisData["job-result-error"][
      "oht-error-percentage"
    ].toFixed(2)
  );
  newData.push(
    dashboardStore.jobResultAnalysisData["job-result-error"][
      "facility-error-percentage"
    ].toFixed(2)
  );
  error.value = newData;
  ...
  newData = [];
  newData.push(
    dashboardStore.jobResultAnalysisData["job-result-error"]["oht-error"]
  );
  newData.push(
    dashboardStore.jobResultAnalysisData["job-result-error"]["facility-error"]
  );
