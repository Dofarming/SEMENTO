...
from config import all_nodes, all_paths, path_and_before_path_info, length_info, facility_length_info, facility_length_info_excel, df_cols, THRESHOLD
...
anomaly_detection_model = load_model('./ai_models/anomaly_detection_model.h5', compile=False)
anomaly_detection_model.compile(optimizer=Adam(), loss='MAE')  # 적절한 손실 함수를 지정합니다.
...
async def data_preprocessing_by_time_with_detection_AI(
    df: pd.DataFrame,
):
    try:
      # MinMaxScailing
        ...
        encoder_path2 = encoder_path.transform(anomaly_df[['path']].rename(columns={'path': 'path'}))

        # 인코딩된 결과를 원래의 데이터프레임에 새로운 컬럼으로 추가
        anomaly_df = pd.concat([anomaly_df, encoded_target.add_suffix('_target'), encoded_current.add_suffix('_current'), encoded_next.add_suffix('_next'), encoder_path2.add_suffix('_current'), encoded_status.add_suffix('_status')], axis=1)

        df = pd.concat([df, encoded_target.add_suffix('_target'), encoded_current.add_suffix('_current'),
                             encoded_status.add_suffix('_status'), encoded_next.add_suffix('_next')], axis=1)
        n_ohts = 30
        anomaly_df = anomaly_df.drop(['oht_id', 'oht_connect', 'target_node', 'current_node', 'next_node', 'curr_time', 'status', 'mode', 'start_time', 'path'], axis=1)
        anomaly_df['carrier'] = anomaly_df['carrier'].astype(bool)
        anomaly_df['is_fail'] = anomaly_df['is_fail'].astype(bool)
        anomaly_df['is_idle'] = anomaly_df['is_idle'].astype(bool)
        
        SNAPSHOT_MATRIX = [[anomaly_df.iloc[k*n_ohts + idx] for idx in range(n_ohts)] for k in range(len(df) // n_ohts)]
        snap_pred = anomaly_detection_model.predict(tf.convert_to_tensor(np.array(SNAPSHOT_MATRIX, dtype=np.float32), dtype=tf.float32))
        
        snap_mae = np.mean(np.abs(snap_pred-np.array(SNAPSHOT_MATRIX, dtype=np.float32)), axis = (1,2))
                ...
                if val >= count_criterion:
                    ...
                    for idx, oht in enumerate(ohts_arr):
                        ...
                        if oht['path'] == path_name and oht['current_node'] == \
                            ...
                            # 지난 10초간 count_criterion 이상이면 혼잡/정체라고 판단
                            is_deadlock = True
                            for idx, temp_count in enumerate(path_count_arr[now_second - 10:now_second]):
                                temp_count.setdefault(path_name, 0)
                                if temp_count.setdefault(path_name, 0) < count_criterion or snap_mae[now_second - 10 + idx] <= THRESHOLD:
                                    is_deadlock = False
...
#         encoder_path = ce.BinaryEncoder(cols=['path'])
#         encoder_path.fit(pd.DataFrame(all_paths, columns=['path'])) 
#         encoder_path2 = encoder_path.transform(df[['path']].rename(columns={'path': 'path'}))

#         # 인코딩된 결과를 원래의 데이터프레임에 새로운 컬럼으로 추가
#         df = pd.concat([df, encoded_target.add_suffix('_target'), encoded_current.add_suffix('_current'), encoded_next.add_suffix('_next'), encoder_path2.add_suffix('_current'), encoded_status.add_suffix('_status')], axis=1)

#         n_ohts = 30
#         anomaly_df = df.drop(['oht_id', 'oht_connect', 'target_node', 'current_node', 'next_node', 'curr_time', 'status', 'mode', 'start_time', 'path'], axis=1)
#         anomaly_df['carrier'] = anomaly_df['carrier'].astype(bool)
#         anomaly_df['is_fail'] = anomaly_df['is_fail'].astype(bool)
#         anomaly_df['is_idle'] = anomaly_df['is_idle'].astype(bool)
        
#         SNAPSHOT_MATRIX = [[anomaly_df.iloc[k*n_ohts + idx] for idx in range(n_ohts)] for k in range(len(df) // n_ohts)]
#         pred = anomaly_detection_model.predict(tf.convert_to_tensor(np.array([SNAPSHOT_MATRIX[0]], dtype=np.float32), dtype=tf.float32))
        
#         print(np.mean(np.abs(pred-np.array(SNAPSHOT_MATRIX[0], dtype=np.float32)), axis = (1,2)))
#         return "asb"
